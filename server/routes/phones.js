...
router.post("/", upload.single("image"), async (req, res) => {
  const { title, desc, price, location, phone, showData, createdBy, user_id, status } = req.body;
...
router.get("/myAds/:id", async (req, res) => {
  const {id} = req.params;
  try {
    const myAds = await Phone.find({ user_id: id});
    res.json(myAds);
  } catch (error) {
    res.json({ status: error });
  }
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    await Phone.findByIdAndUpdate(id, approveProduct);
